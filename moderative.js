...
const isTrivialInsideMessage = (content) => TRIGGER_MESSAGES.TRIVIALS.some(trivial => {
...
const isTrivialQuestion = async (message) =>
  !message.author.bot && !message.attachments.size && !deduceCodePresenceInside(message.content)
  && await isMessageCooledDown(message.channel.fetch.bind(message.channel)) && isTrivialInsideMessage(message.content)
